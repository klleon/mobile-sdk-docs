...
v0.8.0 부터 `voiceRecognitionMode` 기본값이 `ALWAYS_ON` 이고 `sttMode` 기본값이 `STT_ONLY` 입니다. 별도 설정 없이 `play()` 호출만으로 마이크 세션이 자동 시작되며, STT 결과는 `sdkState` 의 `STT_RESULT` chat_type 으로 전달됩니다.

```kotlin
// play()는 suspend 함수이므로 코루틴에서 호출
CoroutineScope(Dispatchers.IO).launch {
    klleonOndeviceSdk.play(
        ...
        // useMelSpectrogram = false,          // v0.12.0+ 기본값 (생략 가능)
    )
}
```
...
| `sttMode` | SttMode | X | STT 결과 전달 모드 (v0.8.0+ 기본값 `STT_ONLY`) |
| `useMelSpectrogram` | Boolean | X | 립싱크 파이프라인 모드 (v0.12.0+ 기본값 `false`). `true`면 캐시 발화 API 사용 가능 |
