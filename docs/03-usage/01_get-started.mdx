...
3.  **캐릭터 ID**: 사용할 AI 아바타의 `characterId`가 필요합니다.
...
klleonOndeviceSdk.play(
    ...
    characterId = "character_001",     // 캐릭터 ID
    language = "ko",                   // 언어 (ko, en, ja 등)
    mmapEnabled = false                // 오디오 MMAP 정책 (기본: false)
)
```
...
| `mmapEnabled` | Boolean | X | false | 오디오 MMAP 활성화 (일부 USB 마이크 비호환) |
