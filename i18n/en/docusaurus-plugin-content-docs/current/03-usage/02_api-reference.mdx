...
Wakeword matching is **disabled** by default. It is automatically enabled when `enterStandby()` is called and automatically disabled when `exitStandby()` is called. Even after registering wakewords with `registerWakewords()`, matching only works after entering standby mode.
:::

:::info STT Language and Wakewords
The language set via `play(language)` or `sendChangeLanguage(language)` is also applied as the STT recognition language. Since wakeword matching operates on STT recognition results for that language, **you must register wakewords that the STT for each language can recognize**.

Examples:
- Korean STT → Wakewords like `"영어"`, `"일본어"`, `"니혼고"` (Korean pronunciation)
- Japanese STT → Wakewords like `"韓国語"`, `"英語"`, `"コリアン"` (Japanese recognition)
- English STT → Wakewords like `"Korean"`, `"Japanese"` (English)
:::

### registerWakewords()

Registers wakewords. Replaces all existing wakewords. Registration alone does not activate matching; it becomes active upon entering standby with `enterStandby()`.
...
|-----------|------|-------------|
| `wakewords` | Map\<String, String\> | Wakeword → tag map. When a wakeword is detected, the tag value is delivered via callback |
...
Clears all registered wakewords. After clearing, wakeword matching will not work and all STT results will be delivered as `STT_RESULT`.
...
:::tip
The SDK does not interpret what a wakeword means. Tag values can be freely defined by the app — not just language codes, but any string such as `"mode_quiet"`, `"stop"`, etc.
:::
...
Enters standby mode. Cleans up the entire pipeline (clears queues, discards audio buffers, resets timing) then pauses video decoding and audio playback. Resources are retained.
...
Checks whether the SDK is currently in standby mode.
...
Enables idle timeout. Automatically calls `enterStandby()` after a specified period of inactivity following avatar speech completion (TTS_END). A `STANDBY_ENTER` event is emitted to `sdkState` before entering standby.
...
| `STANDBY_ENTER` | Automatic standby mode entered |
