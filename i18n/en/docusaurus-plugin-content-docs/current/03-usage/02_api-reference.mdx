...
Copies avatar resources to the cache directory, performs YUV conversion, and loads native memory.
...
| `avatarId` | String | X | Avatar ID. Reuses buffer for instant `onSuccess` when re-entering with the same avatar |
...
Releases native YUV buffer memory and resets resource state.
...
1. **`init()`** — Call when avatar selection is complete. Automatically releases previous buffer when changing avatars.
2. **`play()`** — Can only be called after receiving the `onSuccess` callback from `init()`
3. **`deinit()`** — Call when the app is fully terminated. `finish()` preserves the buffer for optimized re-entry with the same avatar.
...
Dynamically changes the language during conversation.
...
Checks if the video is in a paused state.
...
val customConfig = SilenceDetectionConfig.default().copy(
    silenceTimeoutMs = 2000L  // Silence detection time 2 seconds
)
...
Checks if voice recognition is in progress.
...
| Mode | Description |
|------|-------------|
| `DISCRETE` | Single-shot mode (default). Manual start/stop |
| `ALWAYS_ON` | Always listening. Auto-starts after play(), maintains session |
...
Checks if ALWAYS_ON auto-listening is active.
...
Checks if a USB microphone is connected.
...
| `PREPARING_RESPONSE` | Server preparing response |
| `STT_RESULT` | Final speech recognition result |
| `STT_PARTIAL` | Intermediate speech recognition result |
...
Notifies temporary state changes in the speech recognition session. Occurs with STT providers that use token-based authentication (e.g., Azure).

| Status | message value | Meaning | When it occurs |
|--------|---------------|---------|----------------|
| `Preparing` | `Preparing voice recognition` | Recognition start delayed due to token pending | When voice recognition starts without a received token |
...
:::tip
`STT_STATUS` is a temporary status notification. When receiving `Preparing voice recognition` or `Reconnecting voice recognition`, show a toast notification to the user, and dismiss it naturally when `Voice recognition ready` is received.
:::
...
class MainActivity : ComponentActivity() {
    ...
    override fun onDestroy() {
        super.onDestroy()
        klleonOndeviceSdk.finish()  // Stop SDK (buffer retained for reuse)
        ResourceManager.deinit()    // Fully release buffer on app exit
    }
}
```
