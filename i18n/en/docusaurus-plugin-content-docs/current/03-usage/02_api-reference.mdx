...
```kotlin
// Managed as a singleton (recommended)
object SdkConfig {
...
| `avatarId` | String | No | Avatar ID. When re-entering with the same avatar, buffers are reused and `onSuccess` returns immediately |
| `onSuccess` | () -> Unit | Yes | Resource ready callback |
...
:::info Buffer Reuse
When `init()` is called again with the same `avatarId`, the already-prepared buffers are reused and `onSuccess` is returned immediately. When switching to a different avatar, the previous buffers are released and new ones are allocated.
:::
...
1. **`init()`** -- Call when avatar selection is complete. When switching avatars, previous buffers are automatically released.
2. **`play()`** -- Can only be called after the `onSuccess` callback of `init()` is received
3. **`deinit()`** -- Call on full app shutdown. `finish()` retains buffers to optimize re-entry with the same avatar.
...
Stops the SDK and cleans up all resources.
...
Checks whether the video is in a paused state.
...
| `ALWAYS_ON` | Always listening. Auto-starts after play(), session maintained |
...
Notifies temporary state changes of the voice recognition session. Occurs with STT providers that use token-based authentication (such as Azure).

| Status | message Value | Meaning | When It Occurs |
|--------|---------------|---------|----------------|
| `Preparing` | `Preparing voice recognition` | Recognition start delayed due to waiting for token | When voice recognition starts without a received token |
...
:::tip
`STT_STATUS` is a temporary status notification. When receiving `Preparing voice recognition` or `Reconnecting voice recognition`, display a toast or similar notification to the user, and dismiss it naturally when `Voice recognition ready` is received.
:::
...
|-------|------|---------|-------------|
| `silenceThreshold` | Float | 0.02 | Silence detection level threshold |
| `speechThreshold` | Float | 0.05 | Speech detection level threshold |
...
class MainActivity : ComponentActivity() {
    ...
    override fun onDestroy() {
        super.onDestroy()
        klleonOndeviceSdk.finish()  // Stop SDK (buffers retained for reuse)
        ResourceManager.deinit()    // Fully release buffers on app exit
    }
}
```
