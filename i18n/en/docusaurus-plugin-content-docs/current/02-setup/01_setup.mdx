...
These files should be placed in the `assets` (`src/main/assets`) folder and will be copied to the cache directory at runtime.

### 1-2. AAR File

- Copy the Klleon SDK AAR file to `app/libs`.
...
- Declare the New Relic plugin in your top-level build.gradle(.kts) file
   ```
    plugins {
   ...
   ```
- Apply the plugin explicitly in your app-level build.gradle(.kts)
   ```
      plugins {
        id("com.newrelic.agent.android")
      }
   ```
- Add a newrelic.properties file to your app-level directory
