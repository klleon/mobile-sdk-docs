...
`init()` automatically detects avatar changes. For the same avatar, buffers are immediately reused; for a different avatar, previous buffers are released and new ones are allocated.
...
class CharacterSelectActivity : ComponentActivity() {
    ...
    fun onAvatarSelected(avatarMp4: String, avatarNpz: String, avatarId: String) {
        ...
        // Different avatar: release previous buffer + allocate new
        ResourceManager.init(
            context = this,
