...
class MainActivity : ComponentActivity() {
    ...
    private fun initializeApp() {
        ...
        // Resource initialization (cache copy + YUV conversion + memory loading)
...
`init()` automatically detects avatar changes. For the same avatar, it instantly reuses the buffer; for a different avatar, it releases the previous buffer and allocates a new one.

```kotlin
class CharacterSelectActivity : ComponentActivity() {

    fun onAvatarSelected(avatarMp4: String, avatarNpz: String, avatarId: String) {
        // init() compares avatarId to automatically handle buffer reuse/switching
        // Same avatar: instant onSuccess (0ms)
