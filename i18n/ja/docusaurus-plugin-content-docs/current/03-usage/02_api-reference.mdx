...
```kotlin
klleonSdkView.setRenderRotation(270) // 0, 90, 270度に対応
```
...
| `avatarId` | String | X | アバターID。同一アバターへの再入時はバッファを再利用して即座に`onSuccess`を返します |
...
resourceManager.init(
    ...
    onSuccess = {
        // 準備完了 — play()はsuspendのためコルーチンから呼び出します
...
:::info バッファの再利用
同一アバター（`avatarId`）で`init()`を再呼び出しすると、すでに準備されたバッファを再利用して即座に`onSuccess`を返します。別のアバターに切り替える場合は、以前のバッファを解放してから新たに割り当てます。
:::

### deinit()

ネイティブYUVバッファメモリを解放し、リソースの状態を初期化します。
...
:::tip 冪等性の保証
`deinit()`は複数回呼び出しても安全です。すでに解放された状態で呼び出すとno-opとなります。
:::

### 呼び出しルール

1. **`init()`** — アバター選択完了時に呼び出します。アバター変更時は以前のバッファを自動解放します。
2. **`play()`** — `init()`の`onSuccess`コールバックが呼ばれた場合にのみ呼び出し可能です
3. **`deinit()`** — アプリ完全終了時に呼び出します。`finish()`はバッファを保持して同一アバターへの再入を最適化します。
...
AIアバターのビデオ再生を開始します。`suspend`関数のため、コルーチン内で呼び出す必要があります。
...
SDKを停止し、すべてのリソースをクリーンアップします。`suspend`関数のため、コルーチン内で呼び出す必要があります。
内部バッファは同一アバターへの再入時の再利用のために保持されます（ソフトリセット）。
...
進行中の`finish()`が完了するまで待機します。終了中でない場合は即座に返されます。
`finish()`後すぐに`play()`を再呼び出しする場合、`ResourceManager.init()`の前に呼び出してリソースの競合を防止します。

```kotlin
// 素早い再入時
CoroutineScope(Dispatchers.Default).launch {
    klleonOndeviceSdk.awaitFinish()  // 以前の終了完了を待機
...
STT認識結果から登録されたwakewordを検出し、コールバックを伝達します。Wakewordがマッチングされると、該当STT結果は通常の会話入力（`STT_RESULT`）から除外されます。

:::note Standby連動ポリシー
Wakewordマッチングはデフォルトで**無効化**状態です。`enterStandby()`呼び出し時に自動的に有効化され、`exitStandby()`呼び出し時に自動的に無効化されます。`registerWakewords()`でwakewordを登録しても、standbyモードに進入しないとマッチングは動作しません。
:::
...
例：
- 韓国語STT → `"영어"`、`"일본어"`、`"니혼고"` などの韓国語発音wakeword
- 日本語STT → `"韓国語"`、`"英語"`、`"コリアン"` などの日本語認識wakeword
- 英語STT → `"Korean"`、`"Japanese"` などの英語wakeword
:::

### registerWakewords()

Wakewordを登録します。既存のwakewordを全て置き換えます。登録だけではマッチングは動作せず、`enterStandby()`進入時に有効化されます。
...
| パラメータ | 型 | 説明 |
|---------|------|------|
| `wakewords` | Map\<String, String\> | wakeword → タグマップ。Wakewordが検出されるとタグ値がコールバックで伝達されます |
...
登録されたwakewordを全て初期化します。初期化後はwakewordマッチングが動作せず、すべてのSTT結果が`STT_RESULT`として伝達されます。
...
| フィールド | 型 | 説明 |
|------|------|------|
| `wakeword` | String | マッチングされたwakeword原文（例：`"니혼고"`） |
| `tag` | String | 登録時に指定したタグ（例：`"ja_jp"`） |
| `fullText` | String | STTが認識した全体テキスト |
...
:::tip
SDKはwakewordが何を意味するかには関与しません。タグ値はアプリが自由に定義でき、言語コード以外にも`"mode_quiet"`、`"stop"`などどのような文字列でも使用可能です。
:::
...
待機画面モードに進入します。パイプライン全体をクリーンアップ（キューのクリア、オーディオバッファの破棄、タイミングの初期化）した後、ビデオデコードとオーディオ再生を一時停止します。リソースは維持されます。
...
idle timeoutを有効化します。アバター発話終了（TTS_END）後、指定時間の間無活動の場合に自動的に`enterStandby()`を呼び出します。`STANDBY_ENTER`イベントが`sdkState`に発行された後、待機画面に進入します。
...
:::info
`play()`呼び出しの前後いつでも呼び出し可能です。呼び出さない場合、自動待機画面機能は無効化状態です。
:::

### disableIdleTimeout()

idle timeoutを無効化します。有効化されたタイマーがある場合はキャンセルされます。
...
アバターが応答準備中または発話中であれば`true`。`PREPARING_RESPONSE`受信時に`true`、`TTS_END`または`ERROR`受信時に`false`に切り替わります。
...
音声認識を開始します。マイク録音を開始し、WebSocketでSTART_VOICE信号を送信します。無音が検出されると自動的に録音を終了し、END_VOICE信号を送信します。
...
SDK状態の更新をJSON文字列で発行するSharedFlowです。
...
| `SPEECH_END` | アバター発話終了 |
| `STANDBY_ENTER` | 自動待機画面モード進入 |
| `STOP_RESPONSE_MESSAGE` | 現在のアバター発話中断 |

#### STT_STATUSの詳細

音声認識セッションの一時的な状態変化を通知します。トークンベースの認証を使用するSTTプロバイダー（Azureなど）で発生します。

| 状態 | message値 | 意味 | 発生タイミング |
|------|------------|------|-----------|
| `Preparing` | `音声認識準備中` | トークン待機中のため認識開始が遅延しています | トークン未受信状態で音声認識開始時 |
| `Recovering` | `音声認識再接続中` | 認証失敗によりトークン再発行を待機中です | 認識中に認証トークン期限切れ時 |
| `Ready` | `音声認識準備完了` | 復旧完了、音声認識再開 | トークン再発行後の認識再開時 |

:::tip
`STT_STATUS`は一時的な状態通知です。`音声認識準備中`または`音声認識再接続中`を受信した場合はトーストなどでユーザーに案内し、`音声認識準備完了`を受信した際に自然に解除してください。
:::
...
| `SilenceDetectionConfig.noisyEnvironment()` | 騒音環境（閾値上昇: silence 0.05、speech 0.10） |
...
| プロパティ | 型 | 説明 |
|---------|------|------|
| `isStopping` | Boolean | SDKが現在終了中かどうか（`finish()`進行中なら`true`） |
...
`play()`と`finish()`は`suspend`関数です。Activityのライフサイクルではコルーチンでラップして呼び出します。
...
class ChatActivity : ComponentActivity() {
    ...
    override fun onDestroy() {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            sdk.finish()  // SDK停止（バッファは再利用のために保持）
        }
...
`finish()`後すぐに`play()`を呼び出すと、リソースの競合が発生する可能性があります。
`awaitFinish()`を使用して、以前の終了が完了した後に進めてください：

```kotlin
// キャラクター選択画面で新しいアバターを開始する場合
CoroutineScope(Dispatchers.Default).launch {
    sdk.awaitFinish()  // 以前のfinish()が進行中なら完了を待機
