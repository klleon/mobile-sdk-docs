...
アバターリソースをキャッシュディレクトリにコピーし、YUV変換およびネイティブメモリへの読み込みを行います。
...
| `avatarId` | String | X | アバターID。同一アバターで再入する際にバッファを再利用して即座に`onSuccess`を返します |
...
:::info バッファ再利用
同一アバター（`avatarId`）で`init()`を再呼び出しすると、既に準備されたバッファを再利用して即座に`onSuccess`を返します。異なるアバターに切り替える際は、以前のバッファを解放して新しく割り当てます。
:::
...
:::tip 冪等性保証
`deinit()`は複数回呼び出しても安全です。既に解放された状態で呼び出すとno-opです。
:::

### 呼び出し規則

1. **`init()`** — アバター選択完了時に呼び出します。アバター変更時は以前のバッファを自動解放します。
2. **`play()`** — `init()`の`onSuccess`コールバックが来た時のみ呼び出し可能です
3. **`deinit()`** — アプリ完全終了時に呼び出します。`finish()`はバッファを保持して同一アバター再入を最適化します。
...
ビデオデコーディングを一時停止します。リソースは保持されます。
...
SDK状態のアップデートをJSON文字列として発行するSharedFlowです。
...
| `WSS_RETRY_OVER` | 最大再試行超過 |
| `WSS_NOT_CONNECTED` | 未接続状態 |
| `ACTIVATE_VOICE` | 音声入力受信準備完了 |
| `TEXT` | サーバーからテキスト応答受信 |
| `RESPONSE_IS_ENDED` | サーバー応答ストリーム終了 |
| `TTS_START` | TTS音声再生開始 |
| `TTS_END` | TTS音声再生完了（発話終了検知に使用） |
| `SHOW_AVATAR` | アバター表示準備完了 |
| `USER_SPEECH_STARTED` | ユーザー発話開始検知 |
| `USER_SPEECH_STOPPED` | ユーザー発話終了検知 |
...
|------|------------|------|-----------|
| `Preparing` | `音声認識準備中` | トークン待機中で認識開始が遅延 | トークン未受信状態で音声認識開始時 |
| `Recovering` | `音声認識再接続中` | 認証失敗でトークン再発行待機中 | 認識中に認証トークン期限切れ時 |
...
レコーダー状態をリアルタイムでサブスクライブできるStateFlowです。
...
class MainActivity : ComponentActivity() {
    ...
    override fun onDestroy() {
        super.onDestroy()
        klleonOndeviceSdk.finish()  // SDK停止（バッファは再利用のために保持）
        ResourceManager.deinit()    // アプリ終了時にバッファを完全解放
    }
}
```
